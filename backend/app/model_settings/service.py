...
from .secrets import ModelSecretStore, default_secret_store
...
def load_effective_model_configuration(
    *,
    ...
    resolved_secrets = secrets or default_secret_store()
    data = base.model_dump(mode="python")
    explicit_fields = base.model_fields_set
    if "model_default_provider" not in explicit_fields:
    ...
    for provider in ModelProvider:
        ...
        api_style_field = f"{prefix}_api_style"
        key_field = _KEY_FIELDS[provider]
        if model_field not in explicit_fields:
            data[model_field] = item.model
        if base_url_field not in explicit_fields:
            data[base_url_field] = item.base_url
        if (
            provider is not ModelProvider.ANTHROPIC
            and api_style_field not in explicit_fields
        ):
            data[api_style_field] = item.api_style
        if key_field not in explicit_fields:
            secret = resolved_secrets.get(provider.value)
            if secret:
                data[key_field] = SecretStr(secret)
