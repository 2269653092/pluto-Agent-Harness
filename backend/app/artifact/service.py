...
logger = logging.getLogger("pluto.artifact")
...
# 权威 MIME 表：Windows 注册表常把这些常见扩展映射成本地应用关联
# （如 .csv → application/vnd.ms-excel），导致 Artifact 的 mime_type 不稳定。
# 这里显式固定跨平台一致的类型，其余扩展再回退 mimetypes.guess_type。
_MIME_OVERRIDES = {
    ".csv": "text/csv",
...
def _guess_mime_type(filename: str) -> str:
    """优先查权威表，其余交给 mimetypes（Windows 上再补注册表回退）。"""
