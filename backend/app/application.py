"""Application：Pluto 的 composition root（统一依赖装配）。

CLI（``app.models.chat``）与 Pluto Host（``app.server``）共用这一份
...
from app.agent.hierarchy import PLANNER_SYSTEM_PROMPT, AgentHierarchySettings
...
logger = logging.getLogger("pluto.application")
...
DEFAULT_SYSTEM_PROMPT = (
    "你是 Pluto，一个本地运行的智能助理。请使用用户的语言回答。"
...
class Application:
    ...
    def __init__(
        ...
        *,
        ...
        hierarchy_settings: AgentHierarchySettings | None = None,
        ...
        self.hierarchy_settings = hierarchy_settings or AgentHierarchySettings()
        ...
        self.worker_registry: ModelAdapterRegistry | None = None
        self.worker_provider: str | None = None
        ...
        if registry is not None:
            ...
            planner_settings = (
                self.hierarchy_settings.planner_model_settings(self.settings)
                if self.hierarchy_settings.agent_hierarchy_enabled
                else self.settings
            )
            self.registry = ModelAdapterRegistry(planner_settings)
            selected = select_provider(
                planner_settings,
                provider.value if isinstance(provider, ModelProvider) else provider,
            )
            self.provider = selected.value
            config = planner_settings.provider_config(selected)
            self.model = model or config.model
            if self.hierarchy_settings.agent_hierarchy_enabled:
                worker_settings = self.hierarchy_settings.worker_model_settings(
                    self.settings,
                    planner=planner_settings,
                )
                worker_provider = select_provider(worker_settings, None)
                worker_config = worker_settings.provider_config(worker_provider)
                self.worker_registry = ModelAdapterRegistry(worker_settings)
                self.worker_provider = worker_provider.value
                self.worker_model = worker_config.model
    ...
    async def start(self) -> None:
        ...
        # Computer Runtime 可注入 Windows 实现或测试 Fake；未注入则不注册，
        ...
        if self._computer_runtime is not None:
            ...
            # Runtime 可选实现 start；FakeComputerRuntime 没有该方法。
        ...
        runtime = AgentRuntime(
            ...
            worker_model_registry=self.worker_registry,
            worker_provider=self.worker_provider,
            worker_model=self.worker_model,
            worker_max_steps=self.hierarchy_settings.worker_max_steps,
            worker_max_tool_rounds=(
                self.hierarchy_settings.worker_max_tool_rounds
            ),
            worker_max_output_tokens=(
                self.hierarchy_settings.worker_max_output_tokens
            ),
        )

        run_store = SQLiteRunStore(database)
        # Run 终态 finalizer 顺序：先 end ComputerSession（清 target/snapshot + 通知
        # Runtime 清理状态），再 release Machine Lease。
        ...
        self.active_model_roles = {
            ...
            "worker": {
                "enabled": self.worker_registry is not None,
                "provider": self.worker_provider or self.provider,
                "model": self.worker_model or self.model,
            },
            "summary": {
    ...
    async def close(self) -> None:
        ...
        if self.worker_registry is not None:
            await self.worker_registry.close()
        self._started = False
