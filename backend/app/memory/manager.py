...
from app.platform import lock_file, unlock_file
...
logger = logging.getLogger("pluto.memory.manager")

CORE_MEMORY_MESSAGE_NAME = "pluto_core_memory"
MEMORY_INDEX_MESSAGE_NAME = "pluto_memory_index"
MEMORY_POLICY_MESSAGE_NAME = "pluto_memory_policy"
...
class MemoryManager:
    ...
    async def _mutation_guard(self) -> AsyncIterator[None]:
        """串行化同一实例，并用文件锁协调同目录的进程与其它 Manager。

        POSIX 用 flock，Windows 用 msvcrt.locking（统一封装在 app.platform），
        语义一致：跨句柄 / 跨进程互斥，保证 INDEX 与 active 文件的原子更新。
        """
