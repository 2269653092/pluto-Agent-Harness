...
from app.platform import native_shell, shell_safe_environment
...
class ShellCommandTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._sandbox_supervisor is None:
            ...
            environment = shell_safe_environment()
            shell_command, shell_prefix = native_shell()
            launch = self._sandbox_supervisor.prepare_launch(
                command=shell_command,
                args=(*shell_prefix, command),
                env=environment,
                cwd=str(cwd),
        ...
        return {
            "command": command,
            # 用本机路径表达（Windows 是 "\\"），方便模型直接拿去写命令；
            # POSIX 上 str() 与 as_posix() 等价，行为不变。
...
def _terminate_process(process: asyncio.subprocess.Process) -> None:
    ...
    try:
        ...
        else:  # pragma: no cover - Windows 没有进程组信号，改用进程树终止
...
def _terminate_process_tree(pid: int) -> bool:
    """Windows：连同子进程一起终止，返回是否成功下发终止命令。"""
