...
logger = logging.getLogger("pluto.server")
...
def computer_screenshot(observation_id: str, request: Request) -> Response:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="not found")
    # is_relative_to 是跨平台判断（Windows 分隔符是 "\\"，不能用 startswith + "/"）。
...
def create_app(
    ...
    *,
    ...
    async def lifespan(app: FastAPI):
        ...
        logger.info(
            "Pluto Host started · provider=%s · model=%s",
        ...
        try:
            ...
            logger.info("Pluto Host stopped")
