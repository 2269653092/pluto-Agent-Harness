"""Pluto Host 入口：``python -m app.server``。"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--database",
        help="SQLite database path (default: backend/.pluto/pluto.db).",
    )
    ...
    parser.add_argument(
        "--disable-computer",
        action="store_true",
        help="Disable the Windows Computer Runtime.",
    )
    return parser
...
async def _serve(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if not restart_requested:
            return 0
        logging.getLogger("pluto.server").info(
            "Restarting Pluto Host with saved configuration"
        )
