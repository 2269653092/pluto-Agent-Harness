...
async def test_shell_command_runs_and_captures_output(tmp_path) -> None:
    ...
    result = await executor.execute(
        ToolCall(
            id="sh-1",
            name="run_shell_command",
            arguments={"command": "echo hello pluto"},
        )
    )
    ...
    assert "hello pluto" in output["stdout"]
...
async def test_shell_command_uses_working_directory(tmp_path) -> None:
    ...
    result = await executor.execute(
        ToolCall(
            id="sh-4",
            name="run_shell_command",
            arguments={"command": "cd", "working_directory": "."},
        )
    )
