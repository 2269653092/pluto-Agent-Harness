...
def _notepad() -> ActiveApp:
    return ActiveApp(name="Notepad", pid=111)


def _vs_code() -> ActiveApp:
    return ActiveApp(name="Code", pid=222)
...
def test_end_clears_target_and_snapshot() -> None:
    ...
    session.begin_target(_notepad())
    session.attach_snapshot(_observation("obs-1", _notepad()))
...
def test_cross_run_target_never_leaks() -> None:
    """Run A 绑定 Notepad 并结束；Run B 开始后绝不能继承 Notepad。"""

    manager = ComputerSessionManager()
    run_a = manager.begin("run-a")
    run_a.begin_target(_notepad())
    run_a.attach_snapshot(_observation("obs-a", _notepad()))
    manager.end("run-a")
