...
def test_explicit_environment_settings_override_saved_model_config(
    tmp_path: Path,
) -> None:
    store = ModelSettingsStore(tmp_path / "models.json")
    secrets = FakeSecrets(values={"deepseek": "credential-key"})
    ModelSettingsService(
        store=store,
        secrets=secrets,
        base_settings=_base_settings(),
    ...
    secrets.reads.clear()
    environment_settings = ModelSettings(
        _env_file=None,
        model_default_provider=ModelProvider.DEEPSEEK,
        deepseek_api_key=SecretStr("env-key"),
    ...
    effective = load_effective_model_configuration(
        store=store,
        secrets=secrets,
        base_settings=environment_settings,
    )

    assert effective.settings.model_default_provider is ModelProvider.DEEPSEEK
    assert effective.settings.deepseek_api_key == SecretStr("env-key")
    assert effective.settings.deepseek_model == "deepseek-env-model"
    assert effective.settings.deepseek_base_url == "https://api.deepseek.com/"
    assert "deepseek" not in secrets.reads
