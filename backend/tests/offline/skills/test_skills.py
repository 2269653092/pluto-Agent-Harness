    ...
    *,
    ...
    try:
        ...
        if getattr(exc, "winerror", None) == 1314:
            pytest.skip("Windows 未启用开发者模式，无法创建符号链接")
        raise
...
def test_safe_skill_dir_rejects_escape_and_symlink(tmp_path: Path) -> None:
    ...
    outside.mkdir()
    _symlink_or_skip(tmp_path / "link", outside, target_is_directory=True)
...
def test_safe_skill_file_rejects_symlink(tmp_path: Path) -> None:
    ...
    _symlink_or_skip(skill_dir / "SKILL.md", target)
...
def test_safe_skill_resource_rejects_symlink_and_directory(
    ...
    target.write_text("x", encoding="utf-8")
    _symlink_or_skip(skill_dir / "refs" / "link.txt", target)
