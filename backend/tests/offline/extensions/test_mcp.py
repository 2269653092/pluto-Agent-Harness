...
async def test_missing_environment_reference_fails_only_that_server(
    ...
    monkeypatch.delenv("PLUTO_MISSING_MCP_KEY", raising=False)
    config = MCPServerConfig(
        name="missing_env",
        command=sys.executable,
        args=("-c", "pass"),
        env={"API_KEY": "${PLUTO_MISSING_MCP_KEY}"},
    )
    registry = ToolRegistry()
    ...
    assert statuses[0].state is MCPServerState.FAILED
    assert "PLUTO_MISSING_MCP_KEY" in (statuses[0].error or "")
...
def test_mcp_environment_does_not_inherit_unlisted_host_secret(
    ...
    monkeypatch.setenv("PLUTO_HOST_SECRET", "must-not-leak")
    ...
    assert "PLUTO_HOST_SECRET" not in environment
