...
function createApprovalWindow(): void {
  ...
  const win = new BrowserWindow({
    ...
    show: false,
    frame: false,
    // 审批卡片不抢焦点，避免与 Computer Runtime 恢复目标应用的动作竞态。
    focusable: false,
    resizable: false,
    movable: true,
    alwaysOnTop: true,
...
function createWindow(): void {
  ...
  win.on('closed', () => {
    ...
    // 审批窗口常驻且默认隐藏；若不主动销毁，它会让 Electron 在主窗口
    // 关闭后继续存活，连带使 Vite 和 TypeScript watch 无法退出。
    if (approvalWindow !== null && !approvalWindow.isDestroyed()) {
...
ipcMain.handle('pluto:open-external', async (_event, url: unknown) => {
  if (typeof url !== 'string' || !isHttpUrl(url)) return false
  await shell.openExternal(url)
  return true
})

ipcMain.on('pluto:notify', (_event, payload: unknown) => {
...
ipcMain.on('pluto:approval-set-visible', (_event, visible: unknown) => {
  if (visible !== true && visible !== false) return
  setApprovalVisible(visible)
})

ipcMain.on('pluto:approval-set-size', (_event, height: unknown) => {
  if (approvalWindow === null || approvalWindow.isDestroyed()) return
